...
async def send_to_discord(resolved, msg):
    ...
    try:
        # resolved is always from NUMBER_MAP: can be a channel ID or user ID (both as string)
        if resolved.isdigit():
            # Try channel first
        ...
        print(f"❌ Invalid target: ID '{resolved}' is not a digit")
